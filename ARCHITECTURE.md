...
└──────────────┬──────────────────────────────┘
    ...
    ▼          ▼              ▼          ▼          ▼
┌────────┐ ┌────────┐ ┌────────────┐ ┌────────┐ ┌────────────┐
│  Auth  │ │  User  │ │ Application│ │   AI   │ │   MinIO    │
...
| offer-application | 8083 | 投递管理、面试管理、Offer 管理、Pipeline 流水线 | offer_application |
| offer-ai | 8084 | 智能助手（DeepSeek API 对话） | 无 |

> **为什么不做成 10 个微服务：** 10 个微服务一个人开发，光环境配置就消耗大半时间。公司本质上依赖用户，岗位依赖公司，投递/面试/Offer 属于同一条业务线——拆到 5 个（含 AI）在当前阶段合理。notification 没有强依赖业务方，延后。
